...
const EMPTY_ADDRESS = {
  label: 'Casa', recipient_name: '', phone: '', address: '', city: '', notes: '', latitude: '', longitude: '',
};
  ...
  return (
    <form className="account-address-form" onSubmit={onSubmit}>
      ...
      <div className="form-row account-form-grid">
        ...
        <div className="form-group"><label>Ciudad</label><input className="input" required value={value.city} onChange={(e) => set('city', e.target.value)} /></div>
      </div>
...
export default function CustomerAccount() {
  ...
  return (
    <div className="account-page">
      <div className="account-heading"><div><span className="section-kicker">Mi cuenta</span><h1>Hola, {customer.name || customer.email}</h1><p>Administra tus datos, direcciones y pedidos desde un solo lugar.</p></div><button className="btn" type="button" onClick={() => logout()}>Cerrar sesión</button></div>
      {error && <div className="alert alert-error" role="alert">{error}</div>}
