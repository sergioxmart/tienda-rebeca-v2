  ...
  return (
    <div>
      ...
      <section className="orders-reservations-section">
        ...
        {reservations.length === 0 ? <p className="form-hint">Todavía no hay solicitudes de reserva.</p> : <div className="table-wrap"><table className="data-table"><thead><tr><th>Reserva</th><th>Producto</th><th>Cliente</th><th>Fechas</th><th>Estado</th><th>Valor</th></tr></thead><tbody>{reservations.map((reservation) => <tr key={reservation.id}><td><strong>{reservation.reservation_number}</strong></td><td>{reservation.product_name}<small className="table-subtext">{reservation.variant_sku || 'Variante estándar'}</small></td><td>{reservation.customer_name}<small className="table-subtext">{reservation.customer_email} · {reservation.customer_phone}</small></td><td>{String(reservation.use_date).slice(0, 10)} → {String(reservation.pickup_date).slice(0, 10)}</td><td><span className="badge">{reservation.status === 'lead' ? 'Lead' : reservation.status === 'confirmed' ? 'Confirmada' : reservation.status === 'pending' ? 'Pendiente' : reservation.status}</span></td><td>{formatCOP(reservation.quoted_amount)}</td></tr>)}</tbody></table></div>}
      </section>
    </div>
  );
}
...
const EMPTY_RESERVATION = { reservation_id: '', product_id: '', variant_id: '', requested_type: 'alquiler', use_date: '', pickup_date: '', status: 'confirmed', quoted_amount: '', payment_method: '', shipping_method: '', notes: '' };
  ...
  return (
    <form className="manual-record-form" onSubmit={submit}>
      ...
      <section className="manual-record-section">
        ...
        <div className="manual-record-grid">
          <div className="form-group"><label>Método de pago</label><select className="select" value={kind === 'reservation' ? reservation.payment_method : paymentMethod} onChange={(event) => kind === 'reservation' ? setReservation((current) => ({ ...current, payment_method: event.target.value })) : setPaymentMethod(event.target.value)}><option value="transferencia">Transferencia</option><option value="efectivo">Efectivo</option><option value="contraentrega">Contraentrega</option><option value="whatsapp">WhatsApp</option><option value="otro">Otro</option></select></div>
